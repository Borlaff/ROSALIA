...
def launch_irsa_query(ra, dec, wavelength, year, day, obslocin=3):
    ...
    while not (os.path.exists(outfile) and os.path.getsize(outfile)) > 0:
        output = rs.utils.execute_cmd(cmd)
    return(outfile)
...
def read_irsa_query(xml_query_name):
    ...
    output = rs.utils.execute_cmd("rm " + xml_query_name)
    return(d)
...
def irsa_query(ra, dec, wavelength, year, day, obslocin):
    ...
    num_cores = np.max(np.array([multiprocessing.cpu_count(), 1]))
    pool = multiprocessing.Pool(processes=num_cores)
    query = pool.starmap(irsa_query_single, arguments)
    pool.close()
    pool.join() 
